...
export async function fetchDB(): Promise<DatabaseState> {
  ...
  if (!res.ok) {
    throw new Error("Failed to fetch Share Tour database from backend API");
  }
  return res.json();
}
...
export async function getMidtransSnapToken(params: {
  ...
  if (!res.ok) {
    const errorData = await res.json();
    throw new Error(errorData.error || errorData.details || "Failed to generate Midtrans Snap Token");
  }
  return res.json();
}
